...
dispatcher.onGet("/ncco", function(req, res) {
    console.log("Am ajuns aici si am facut pici");
...
dispatcher.onPost("/recording", function(req, res) {
    console.log("AM PRIMIT RECORDING FMMKMMMMMM");
    var parsedUrl = url.parse(req.url, true); // true to get query as object
    var getparams = parsedUrl.query;
    var params = JSON.parse(req.body);
...
pagews.on('connect', function(connection) {
    ...
    console.log((new Date()) + ' Connection (pagews) accepted' + ' - Protocol Version ' + connection.webSocketVersion);
    clients.push(connection);
    connection.on('message', function(message, id) {
        recognizeStream.request.config.languageCode = "en-GB";
    });
...
nexmows.on('connect', function(connection) {
    console.log((new Date()) + ' Connection accepted' + ' (nexmws) - Protocol Version ' + connection.webSocketVersion);
    // Create the stream at the start of the call
    // recognizeStream = new RecognizeStream(connection);
});
